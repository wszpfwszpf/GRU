...
from Denset_efficient import DenseNet
...
def train_epoch(model, loader, optimizer, epoch, num_epochs, print_freq=1):
    ...
    for batch_idx, (in_put, target) in enumerate(loader):
        # Create vaiables
...
def test_epoch(model, loader, print_freq=1, is_test=True):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (in_put, target) in enumerate(loader):
            ...
            correct += (predicted.cuda() == target.cuda()).sum()
            total += in_put.size()[0]
            # measure elapsed time
            batch_time.update(time.time() - end)
            end = time.time()
...
def train(model, train_set, valid_set, test_set, save, num_epochs=30,
    ...
    # Optimizer
    optimizer = torch.optim.SGD(model_wrapper.parameters(), lr=lr, momentum=momentum, nesterov=True,
                                weight_decay=wd)
...
def demo():
    ...
    #     num_epochs (int) - number of epochs for training (default 300)
    #     batch_size (int) - size of minibatch (default 256)
    #     seed (int) - manually set the random seed (default None)

    depth = 100  # depth (int) - depth of the network (number of convolution layers) (default 40)
    data = 'data/cifar10-data'  # data (str) - path to directory where data should be loaded from/downloaded
    save = 'save'  # path to save the model to
    growth_rate = 12
    efficient = True,
    valid_size = 5000
    num_epochs = 30
