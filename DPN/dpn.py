...
class DualPathBlock(nn.Module):
    def __init__(self, in_chs, num_1x1_a, num_3x3_b, num_1x1_c, inc, G, _type='normal'):
        ...
        if self.has_proj:
            self.c1x1_w = self.BN_ReLU_Conv(in_chs=in_chs, out_chs=num_1x1_c + 2 * inc, kernel_size=(1, 1),
    ...
    def forward(self, x):
        data_in = torch.cat(x, 1) if isinstance(x, list) else x
        if self.has_proj:
...
class DPN(nn.Module):
    def __init__(self, num_init_features=64, k_R=96, G=32,
        ...
        blocks['conv1'] = nn.Sequential(
            nn.Conv2d(3, num_init_features, kernel_size=(7, 7), stride=(2, 2), padding=3, bias=False),
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(features, kernel_size=1).view(features.size()[0], -1)
